...
const drawFloor = (fullOne: boolean) => {
    // Draw the room's floor. It can either be a centered one or a full one
    ctx.strokeStyle = "black";
...
const drawRoom = (x: number, y: number, number: string, stand: boolean, width?: number) => {
    ...
    ctx.strokeStyle = Number(number) !== locationId ? "blue" : "yellow";
    ctx.strokeRect(x, y, width ?? 75, 75);

    // Draw the text with the ID
    ctx.fillStyle = Number(number) !== locationId ? "black" : "white";
